...
func NewXenserverHypervisorResource() resource.Resource {
	return &xenserverHypervisorResource{}
}

type xenserverHypervisorResource struct {
	client *citrixdaasclient.CitrixDaasClient
}

// Metadata implements resource.Resource.
func (*xenserverHypervisorResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_xenserver_hypervisor"
}

// Configure implements resource.ResourceWithConfigure.
func (r *xenserverHypervisorResource) Configure(_ context.Context, req resource.ConfigureRequest, _ *resource.ConfigureResponse) {
...
func (*xenserverHypervisorResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	// Retrieve import ID and save to id attribute
	resource.ImportStatePassthroughID(ctx, path.Root("id"), req, resp)
}

// Create implements resource.Resource.
func (r *xenserverHypervisorResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	defer util.PanicHandler(&resp.Diagnostics)

	// Retrieve values from plan
	var plan XenserverHypervisorResourceModel
	...
	connectionDetails.SetConnectionType(citrixorchestration.HYPERVISORCONNECTIONTYPE_XEN_SERVER)
	...
	if err != nil || pwdFormat == nil {
		resp.Diagnostics.AddError(
			"Error creating Hypervisor for XenServer",
...
func (r *xenserverHypervisorResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	defer util.PanicHandler(&resp.Diagnostics)
	...
	if hypervisor.GetConnectionType() != citrixorchestration.HYPERVISORCONNECTIONTYPE_XEN_SERVER {
		resp.Diagnostics.AddError(
			"Error reading Hypervisor",
			"Hypervisor "+hypervisor.GetName()+" is not a XenServer connection type hypervisor.",
		)
		return
	}
...
func (r *xenserverHypervisorResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	defer util.PanicHandler(&resp.Diagnostics)
	...
	editHypervisorRequestBody.SetConnectionType(citrixorchestration.HYPERVISORCONNECTIONTYPE_XEN_SERVER)
	...
	if err != nil || pwdFormat == nil {
		resp.Diagnostics.AddError(
			"Error updating Hypervisor for XenServer",
			"Unsupported password format: "+plan.PasswordFormat.ValueString(),
		)
	}
...
func (r *xenserverHypervisorResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	defer util.PanicHandler(&resp.Diagnostics)
