...
func (r *nutanixHypervisorResourcePoolResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	_, err = UpdateHypervisorResourcePool(ctx, r.client, &resp.Diagnostics, plan.Hypervisor.ValueString(), plan.Id.ValueString(), editHypervisorResourcePool, updateTimeout)
	if err != nil {
		return
	}
...
func (r *nutanixHypervisorResourcePoolResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	for _, hypervisorAndResourcePool := range hypervisorAndResourcePools.GetItems() {
		if hypervisorAndResourcePool.ConnectionType != citrixorchestration.HYPERVISORCONNECTIONTYPE_CUSTOM && hypervisorAndResourcePool.GetPluginId() != util.NUTANIX_PLUGIN_ID {
			continue
		}
